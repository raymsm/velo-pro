...
import { RiderDetails, VehicleProfile, UserSettings, Currency, RideRecord } from '../types';
...
export default function ProfileSettings({ onUpdate, rides }: ProfileSettingsProps) {
      ...
      <div className="space-y-6 pb-20">
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
               <Button variant="outline" className="border-[#2A2D33] text-[9px] font-bold uppercase tracking-widest h-10" onClick={() => RideService.exportData(rides)}>
                      ...
                      if (file) {
                          ...
                          reader.onload = async (ev) => {
                              try {
                                  ...
                                  const count = await RideService.importData(data);
                                  setImportStatus({
                                      type: 'success',
                                      message: `INTELL_SYNC: Successfully imported ${count} ride records.`
                                  });
                                  onUpdate();
                              } catch (err) {
                                  ...
                                  setImportStatus({
                                      type: 'error',
                                      message: `SYNC_FAILED: ${err instanceof Error ? err.message : 'Invalid database schema'}`
                                  });
                              }
                          }
          ...
          {importStatus && (
              <div className={`p-2 rounded-sm border text-[10px] font-mono leading-tight ${
