...
import { Bike, History as HistoryIcon, LayoutDashboard, Fuel, LogOut, Navigation, Play, ShieldAlert, Calendar } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { auth, safeOnAuthStateChanged, safeSignInWithPopup, safeSignOut } from './firebase';
...
export default function App() {
      ...
      navigator.geolocation.getCurrentPosition(async (pos) => {
        try {
          ...
          if (!res.ok) {
            ...
            console.error("Weather sync failed with status:", res.status, errorData);
            return;
          }
  ...
  useEffect(() => {
    const unsubscribeAuth = safeOnAuthStateChanged(auth, (u) => {
      setUser(u);
      setLoading(false);
      if (!u) {
        setRides([]);
  ...
  const handleLogin = async () => {
    try {
      await safeSignInWithPopup(auth);
    } catch (error) {
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                ...
                <TabsContent value="settings" className="m-0">
                     <motion.div key="settings" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                        <ProfileSettings onUpdate={loadRides} rides={rides} />
                    </motion.div>
                </TabsContent>
            </Tabs>
        </AnimatePresence>
